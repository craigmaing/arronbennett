...
def analyze_page(content, filename):
    ...
    external = [l for l in links if l.startswith('http') and 'arronbennettbuilding' not in l]
    
    for link in external[:3]:
        if 'rel=' not in content or f'href="{link}"' not in content.split('rel=')[0]:
            findings['low'].append(f'A11y: External link missing rel: {link[:50]}')
    
    if links:
        findings['passed'].append(f'Links: {len(links)} total, {len(external)} external')
    ...
    if size_kb > 200:
        ...
        findings['passed'].append(f'Performance: Good page size ({size_kb:.0f f} KB)')
...
for page_path in pages[:15]:
    try:
        ...
        rel_path = str(page_path.relative_to(dist)).replace('\index.html', '').replace('\', '/')
        url = rel_path or '/'
